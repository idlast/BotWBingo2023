...
var bingo = function (bingoList, size) {
    ...
    var HIDDEN_SLOTS_PARAM = gup("hiddenSlots"); // 新しく追加
    ...
    $("#bingo tr").on('click', 'td:not(.popout):not(.hidden)',
        function () {
            if ($(this).hasClass('greensquare')) {
                ...
                // divisible by 5? nothing to the right
                if (slot % 5 != 0) {
    ...
    if (EXPLORATION) {
        ...
        if (!isNaN(parsedHiddenSlots)) {
            numHiddenSlots = Math.max(0, Math.min(parsedHiddenSlots, 25)); // 0～25の範囲に制限
        }
    }

    // 隠すスロットをランダムに選択する関数
    function getRandomSlots(totalSlots, numSlots) {
        var slots = [];
        ...
        // 最初の numSlots を返す
        return slots.slice(0, numSlots);
    }

    // 隠すスロットを取得
    var hiddenSlots = [];
    if (EXPLORATION) {
        hiddenSlots = getRandomSlots(25, numHiddenSlots);
    }

    // ビンゴボードをページに表示
    for (i = 1; i <= 25; i++) {
        $('#slot' + i).append(bingoBoard[i].name);
        if (EXPLORATION && hiddenSlots.includes(i)) {
